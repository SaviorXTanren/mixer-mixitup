...
namespace MixItUp.Base.Services.External
{
    ...
    {
        public const string ClientIDWebSocketProperty = "ClientID";
        ...
        private Dictionary<Guid, AdvancedClientWebSocket> websockets = new Dictionary<Guid, AdvancedClientWebSocket>();
        private Dictionary<Guid, Dictionary<string, VTubeStudioWebSocketResponsePacket>> websocketResponses = new Dictionary<Guid, Dictionary<string, VTubeStudioWebSocketResponsePacket>>();

        private Dictionary<Guid, IEnumerable<VTubeStudioModel>> allModelsCache;
        private DateTimeOffset allModelsCacheExpiration = DateTimeOffset.MinValue;

        private Dictionary<Guid, Dictionary<string, IEnumerable<VTubeStudioHotKey>>> modelHotKeyCache = new Dictionary<Guid, Dictionary<string, IEnumerable<VTubeStudioHotKey>>>();
        ...
        public override async Task Disconnect()
        {
            ...
            foreach (var kvp in this.websockets)
            {
                kvp.Value.Disconnected -= Websocket_OnDisconnectOccurred;
        ...
        public async Task<VTubeStudioModel> GetCurrentModel(Guid clientID)
        {
            try
            {
                if (this.IsConnected && this.websockets.TryGetValue(clientID, out AdvancedClientWebSocket webSocket) && webSocket.IsOpen())
                {
                    VTubeStudioWebSocketResponsePacket response = await webSocket.SendAndReceive(new VTubeStudioWebSocketRequestPacket("CurrentModelRequest"));
        ...
        private async Task<bool> ConnectWebSocket(VTubeStudioConnectionModel connection, CancellationToken cancellationToken)
        {
            if (!this.websockets.ContainsKey(connection.ID))
            {
                this.websockets[connection.ID] = new AdvancedClientWebSocket();
                this.websockets[connection.ID].Properties[ClientIDWebSocketProperty] = connection.ID.ToString();

                this.websockets[connection.ID].Disconnected -= Websocket_OnDisconnectOccurred;
                this.websockets[connection.ID].Disconnected += Websocket_OnDisconnectOccurred;

                this.websocketResponses[connection.ID] = new Dictionary<string, VTubeStudioWebSocketResponsePacket>();
            }

            return await this.websockets[connection.ID].Connect(connection.ConnectionURL, cancellationToken);
        }

        private async void Websocket_OnDisconnectOccurred(object sender, WebSocketCloseStatus e)
        {
            // NEED TO ADD ServiceClientBase LOGIC HERE AS GENERAL SERVICE CODE!
            return 0;
        }
    }
}
