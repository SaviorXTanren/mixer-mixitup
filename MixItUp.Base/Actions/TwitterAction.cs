        ...
        protected override async Task PerformInternal(UserViewModel user, IEnumerable<string> arguments)
        {
            if (ChannelSession.Services.Twitter != null)
            {
                if (this.ActionType == TwitterActionTypeEnum.SendTweet)
                {
                    ...
                    {
                        await ChannelSession.Chat.Whisper(ChannelSession.MixerStreamerUser.username, "The tweet you specified can not be sent because it contains an @mention");
                        return;
                    }

                    if (!await ChannelSession.Services.Twitter.SendTweet(tweet, imagePath))
                    {
                        await ChannelSession.Chat.Whisper(ChannelSession.MixerStreamerUser.username, "The tweet you specified could not be sent. Please ensure your Twitter account is correctly authenticated and you have not sent a tweet in the last 5 minutes");
                    }
                }
