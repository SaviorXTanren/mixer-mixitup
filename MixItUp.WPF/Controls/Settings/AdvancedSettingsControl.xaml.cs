        ...
        private async void UnbanAllUsersButton_Click(object sender, RoutedEventArgs e)
        {
            await this.Window.RunAsyncOperation(async () =>
            {
                if (ChannelSession.IsStreamer)
                {
                    ...
                    {
                        foreach (UserWithGroupsModel user in await ChannelSession.MixerStreamerConnection.GetUsersWithRoles(ChannelSession.Channel, MixerRoleEnum.Banned))
                        {
                            await ChannelSession.MixerStreamerConnection.RemoveUserRoles(ChannelSession.Channel, user, new List<MixerRoleEnum>() { MixerRoleEnum.Banned });
                        }
                    }
                }
                else
